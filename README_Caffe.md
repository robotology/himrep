...
Although Caffe can be compiled also on the CPU, it is recommended to run this module on an NVIDIA GPU with Compute Capability higher or equal to 3.0 and CUDA version higher or equal to 5.5 in order to obtain acceptable performance ar runtime. 
...
We do not cover here exaustively the procedure because it is changing quite rapidly (as Caffe is under active developement) and updated and detailed instructions are provided by the developers. We report the procedure we followed to use Caffe library in YARP and iCub's modules on Ubuntu 14.04 LTS.
...
(*) We point out that, in the configuration step:
...
3) Provide the network model definition

Navigate to the folder where you have cloned the 'himrep' repository and copy the 'imagenet_val_cutfc6.prototxt' file (or all himrep context) to the YARP local context directory:

cd himrep/modules/caffeCoder/app/conf
...
If you choose the 'BVLC Reference CaffeNet' model, as we did, you can download the weights (and other data related to the model) by running the command:
